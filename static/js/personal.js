...
(function(){
  ...
  function initialSection(){
    ...
    return params.get('section') || (window.location.hash ? window.location.hash.slice(1) : '') || document.body.dataset.activeSection || 'inicio';
  }
  ...
  document.addEventListener('click', e => {
    ...
    if (e.target.closest('[data-open-logout]')) document.getElementById('logoutDialog')?.showModal();
  });
  ...
  async function loadAccounts(){
    ...
    const options = '<option value="">Seleccione...</option>' + cuentas.map(c => `<option value="${c.id}">${escapeHtml(c.nombre)}</option>`).join('');
    ...
    if (tabla) {
      tabla.innerHTML = cuentas.map(c => `<tr><td>${escapeHtml(c.nombre)}</td><td>${escapeHtml(c.tipo)}</td><td>${currency(c.saldo_inicial)}</td><td><strong>${currency(c.saldo_actual)}</strong></td><td><div class="actions"><button class="btn small" type="button" data-edit-account="${c.id}">Editar</button><button class="btn danger small" type="button" data-delete-account="${c.id}">Eliminar</button></div></td></tr>`).join('') || '<tr><td colspan="5" class="empty">Sin cuentas registradas.</td></tr>';
    }
  ...
  function renderResumenCuentas(cuentas){
    ...
    if (!body) return;
    body.innerHTML = (cuentas || []).map(c => `<tr><td><strong>${escapeHtml(c.nombre)}</strong></td><td>${escapeHtml(c.tipo)}</td><td>${currency(c.ingresos)}</td><td>${currency(c.egresos)}</td><td><strong>${currency(c.saldo_actual)}</strong></td><td>${currency(c.saldo_inicial)}</td></tr>`).join('') || '<tr><td colspan="6" class="empty">Sin cuentas registradas.</td></tr>';
  }

  function statePill(estado){ return `<span class="pill state state-${escapeHtml(estado)}">${escapeHtml(estado)}</span>`; }
  ...
  async function loadMovements(){
    ...
    if (ingresos) ingresos.innerHTML = movimientos.filter(m => m.tipo === 'ingreso').map(m => renderRow(m)).join('') || '<tr><td colspan="8" class="empty">Sin ingresos registrados.</td></tr>';
    if (egresos) egresos.innerHTML = movimientos.filter(m => m.tipo === 'egreso').map(m => renderRow(m)).join('') || '<tr><td colspan="8" class="empty">Sin egresos registrados.</td></tr>';
    if (report) report.innerHTML = movimientos.map(m => renderRow(m, true)).join('') || '<tr><td colspan="9" class="empty">Sin movimientos.</td></tr>';
  }
  ...
  document.querySelectorAll('[data-category-select]').forEach(select => {
    select.addEventListener('change', () => {
      ...
      if (!field && select.value === '__new__') {
        ...
        field.innerHTML = 'Nueva categoría<input name="nueva_categoria" placeholder="Nombre de la categoría" />';
  ...
  document.querySelector('[data-account-form]')?.addEventListener('submit', async e => {
    e.preventDefault();
    try {
      const form = e.target;
      const data = Object.fromEntries(new FormData(form).entries());
      data.saldo_inicial = String(numberValue(data.saldo_inicial));
      await api('/dashboard/api/cuentas', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data) });
      form.reset();
      await loadAccounts();
      await loadDashboard();
  ...
  document.addEventListener('click', async e => {
    ...
    if (editAccountBtn) {
      try {
        const cuenta = await api(`/dashboard/api/cuentas/${editAccountBtn.dataset.editAccount}`);
        ...
        form.elements['saldo_inicial'].value = money.format(Number(cuenta.saldo_inicial || 0));
  ...
  document.getElementById('editAccountForm')?.addEventListener('submit', async e => {
    ...
    try {
      ...
      const data = Object.fromEntries(new FormData(form).entries());
      data.saldo_inicial = String(numberValue(data.saldo_inicial));
      await api(`/dashboard/api/cuentas/${data.id}`, { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data) });
  ...
  document.getElementById('confirmDeleteAccount')?.addEventListener('click', async () => {
    ...
    try {
      await api(`/dashboard/api/cuentas/${deleteAccountId}`, { method:'DELETE' });
  ...
  async function init(){
    try { await Promise.all([loadCategories('ingreso'), loadCategories('egreso'), loadAccounts()]); } catch(err) { showNotice(err.message, 'error'); }
    try { await Promise.all([loadDashboard(), loadMovements()]); } catch(err) { showNotice(err.message, 'error'); }
  }
  init();
