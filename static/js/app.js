(function () {
  ...
  const currency = (value) =>
    "$ " + money.format(Math.max(0, Number(value || 0)));
  ...
  function getAppAlertDialog() {
    ...
    dialog.innerHTML =
      '<div class="modal-content confirm-modal"><h2 data-app-alert-title>Aviso</h2><p data-app-alert-message></p><div class="form-actions"><button class="btn secondary" type="button" data-app-alert-cancel>Cancelar</button><button class="btn" type="button" data-app-alert-accept>Aceptar</button></div></div>';
  ...
  function showAppAlert(message, options = {}) {
    ...
    title.textContent =
      options.title || (isConfirm ? "Confirmar acción" : "Aviso");
  ...
  function bindFormattedInput(input) {
    ...
    input.addEventListener("paste", () =>
      setTimeout(() => formatIntInput(input), 0),
    );
  }
  ...
  document.querySelectorAll("[data-client-select]").forEach((select) => {
    const map = {
      email: select.dataset.targetEmail,
      telefono: select.dataset.targetPhone,
      direccion: select.dataset.targetAddress,
    };
  ...
  document.querySelectorAll("[data-provider-select]").forEach((select) => {
    const map = {
      numero: select.dataset.targetNumber,
      email: select.dataset.targetEmail,
      telefono: select.dataset.targetPhone,
      direccion: select.dataset.targetAddress,
    };
  ...
  document.addEventListener("click", (event) => {
    ...
    if (!event.target.closest("[data-product-search-wrap]")) {
      document.querySelectorAll("[data-product-results]").forEach((box) => {
        box.hidden = true;
        box.closest(".invoice-lines-card")?.classList.remove("search-open");
      });
    }
  });
  ...
  if (itemsScript) {
    try {
      saleItems = JSON.parse(itemsScript.textContent || "[]");
    } catch (e) {
      saleItems = [];
    }
  }
  ...
  function findItem(text) {
    ...
    return (
      saleItems.find(
        (item) =>
          normalize(item.label) === t ||
          normalize(item.nombre) === t ||
          normalize(item.codigo) === t,
      ) || null
    );
  }
  function matchingItems(text) {
    ...
    return saleItems
      .filter((item) => {
        const hay =
          `${item.codigo || ""} ${item.nombre || ""} ${item.descripcion || ""}`.toLowerCase();
  ...
  function fillRowWithItem(row, item) {
    ...
    if (search) {
      search.value = item.label || item.nombre || "";
  ...
  function handleProductOption(event) {
    ...
    if (productOption.dataset.itemId) {
      const item = saleItems.find(
        (x) => String(x.id) === String(productOption.dataset.itemId),
      );
  ...
  document.addEventListener("click", (event) => {
    ...
    if (option) {
      ...
      const item = saleItems.find(
        (x) => String(x.id) === String(option.dataset.itemId),
      );
      fillRowWithItem(row, item);
      const box = row.querySelector("[data-product-results]");
      if (box) {
  ...
  function bindLine(row) {
    ...
    if (search) {
      ...
      search.addEventListener("input", () => {
        clearSelectedItemIfNeeded(row);
        showProductResults(row);
      });
      search.addEventListener("keydown", () =>
        setTimeout(() => positionProductResults(row), 0),
      );
      ...
      remove.addEventListener("click", () => {
        ...
        if (body.querySelectorAll("[data-line-row]").length > 1) row.remove();
          ...
          row.querySelectorAll("input").forEach((input) => {
            ...
            else if (input.matches("[data-price],[data-tax]"))
              input.value = "0";
            else input.value = "";
          });
        }
  ...
  document.querySelectorAll("[data-invoice-form]").forEach((form) => {
    ...
    form.addEventListener("submit", (event) => {
      ...
      form.querySelectorAll("[data-line-row]").forEach((row) => {
        ...
        if (
          row.querySelector("[data-item-id]")?.value &&
          numberValue(row.querySelector("[data-quantity]")?.value) > 0
        )
          valid = true;
      });
      if (!valid) {
  ...
  window.addEventListener(
    ...
    () => {
      document
        .querySelectorAll("[data-product-results]:not([hidden])")
        .forEach((box) =>
          positionProductResults(box.closest("[data-line-row]")),
        );
    },
    true,
  );
  window.addEventListener("resize", () => {
    document
      .querySelectorAll("[data-product-results]:not([hidden])")
      .forEach((box) => positionProductResults(box.closest("[data-line-row]")));
  });
  ...
  function renderCharts() {
    ...
    renderCategoryDonut(
      "donutIngresosChart",
      "donutIngresosSummary",
      data.distribution_ingresos || [],
      "Ingresos",
      "#2468f2",
    );
    renderCategoryDonut(
  ...
  function renderMonthlyChart(rows, labels) {
    ...
    const maxVal = Math.max(
      1,
      ...rows.map((r) => Math.max(r.ventas || 0, r.compras || 0)),
    );
    ...
    rows.forEach((r, idx) => {
      const baseX = padL + idx * group + group * 0.22;
      const bw = Math.max(7, group * 0.18);
      ...
      svg += `<text class="label" x="${padL + idx * group + group * 0.38}" y="${h - 14}" text-anchor="middle">${r.mes}</text>`;
    });
    svg += "</svg>";
  ...
  function renderDonutChart(totals, labels) {
    ...
    if (!target) return;
    const ventas = Number(totals.ventas || 0),
      compras = Number(totals.compras || 0),
      sum = ventas + compras;
    if (sum <= 0) {
      ...
      if (summary)
        summary.innerHTML = `<div class="summary-row"><span>${labels.ventas || "Ventas"}</span><strong>$ 0</strong></div><div class="summary-row"><span>${labels.compras || "Compras"}</span><strong>$ 0</strong></div>`;
      return;
    }
    ...
    if (compras > 0)
      svg += `<path class="donut-segment" data-tip="<strong>${labels.compras || "Compras"}</strong><br>${currency(compras)}<br>${Math.round((compras / sum) * 100)}% del total" d="${donutSegment(130, 130, 82, vAngle, vAngle + cAngle)}" fill="none" stroke="#f4a62a" stroke-width="34" stroke-linecap="round"></path>`;
    ...
    target.innerHTML = svg;
    if (summary)
      summary.innerHTML = `<div class="summary-row"><span><span class="dot ventas"></span> ${labels.ventas || "Ventas"}</span><strong>${currency(ventas)}</strong></div><div class="summary-row"><span><span class="dot compras"></span> ${labels.compras || "Compras"}</span><strong>${currency(compras)}</strong></div>`;
    target.querySelectorAll("[data-tip]").forEach((el) => {
      el.addEventListener("mousemove", (e) => showTip(el.dataset.tip, e));
      el.addEventListener("mouseleave", hideTip);
    });
  ...
  function renderCategoryDonut(targetId, summaryId, rows, label, baseColor) {
    ...
    if (sum <= 0) {
      ...
      if (summary)
        summary.innerHTML = `<div class="summary-row"><span>${label}</span><strong>$ 0</strong></div>`;
      return;
    }
    let angle = 0;
    let svg = `<svg viewBox="0 0 260 260" class="svg-chart" role="img">`;
...
// Verificacion por correo para cambio de contraseña desde Configuracion.
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll('[name="codigo_contrasena"]').forEach((input) => {
    input.addEventListener("input", () => {
  ...
  document
    .querySelectorAll("[data-send-config-password-code]")
    .forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          const data = await postJSON(
            "/api/solicitar-codigo-configuracion",
            {},
          );
          notify(
            data.message || "Código enviado al correo registrado.",
            "Código enviado",
          );
          const form = button.closest("form");
          form?.querySelector('[name="codigo_contrasena"]')?.focus();
        } catch (error) {
